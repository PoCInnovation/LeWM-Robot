...
ADAPTATION MACHINE UNIQUEMENT — comportement planner d'origine préservé
(CEM avec bornes [-1, 1], pas de normalisation d'actions).

Ce qui change (machine) :
    - mode --from-encoded : tourne 100 % sur latents pré-encodés (format v2),
      AUCUN accès Hub ni encodeur — c'est le mode d'éval offline sur cluster ;
    - checkpoints au format dossier (best.pt) produits par 04/05 ;
    - en mode images, l'encodeur suit la config (même famille que 02 —
      indispensable en offline : un seul modèle est pré-téléchargé).

Usage:
    # Éval offline sur latents (recommandé sur cluster) :
    ...
    # Depuis les images du dataset (nécessite encodeur + dataset locaux) :
...
def load_predictor(ckpt_path: Path, device: str):
    ...
    return predictor, fusion, meta


def make_planner(predictor, meta, args):
    cem_cfg = CEMConfig(
...
def resolve_ckpt(args) -> Path:
    for candidate in ([args.ckpt] if args.ckpt else []) + [
        ...
        p = Path(candidate) if Path(candidate).is_absolute() else ROOT / candidate
        if p.exists():
            return p
...
def demo_from_encoded(args, device):
    ...
    planner = make_planner(predictor, meta, args)
    ...
    print(f"\nAction CEM (step 0)          : {actions[0].cpu().numpy().round(3)}")
    print(f"Action vraie du dataset (t0) : {true_action.numpy().round(3)}")
    print(f"Différence absolue           : "
          f"{(actions[0].cpu() - true_action).abs().numpy().round(3)}")
...
def demo_from_images(args, device, cfg):
    ...
    # Même famille/size que 02 (config) — un seul modèle pré-téléchargé offline
    ...
    print(f"\nAction CEM (step 0)  : {actions[0].cpu().numpy().round(3)}")
    print(f"Action vraie (t0)    : {cur['action'].numpy().round(3)}")
